...
* **Export style:** **Always** declare components with **named exports**:
  ...
  *Exception:* use `default export` only where **Next.js requires it** (e.g., `app/**/page.tsx`, `layout.tsx`, route handlers).
...
Absolutely — here’s a drop-in section you can paste into your `copilot-instructions.md` (I’d place it right after **“4) UI, Motion, Icons, Theme”**). I also included two tiny bullets you can add to **House Rules** and **Quality Gates**.
...
export const CtaCard = ({ id, className }: CtaCardProps) => {
  ...
  return (
    <motion.section
      ...
      className={cn("rounded-2xl border p-6 shadow-sm bg-background", className)}
    >
